...
# Timing constants
XRAY_BOOT_MAX   = 0.40   # seconds — kill xray if port not bound by this
...
def build_xray_config(cfg: str, socks_port: int, http_port: int = None,
    ...
    if http_port:
        inbounds.append({"port": http_port, "listen": "127.0.0.1",
                         "protocol": "http",
...
async def test_phase(configs: list[str]) -> list[str]:
    ...
    if not os.path.exists(XRAY_BIN):
        ...
        log("Place xray in the same directory.", 4)
        pm._set(phase="ERROR"); return []
    if not os.access(XRAY_BIN, os.X_OK):
        log(f"xray not executable. Run: chmod +x {XRAY_BIN}", 3)
        pm._set(phase="ERROR"); return []

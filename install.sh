...
if $IS_TERMUX; then
    # In Termux, use $PREFIX/tmp (usually /data/data/com.termux/files/usr/tmp)
    TEMP_DIR="${PREFIX:-$HOME}/tmp"
else
    TEMP_DIR="/tmp"
fi
...
mkdir -p "$TEMP_DIR" 2>/dev/null || {
    echo -e "  ${RD}✗ Cannot create temporary directory: $TEMP_DIR${R}" >&2
    exit 1
}

if [[ ! -w "$TEMP_DIR" ]]; then
    echo -e "  ${RD}✗ Temporary directory is not writable: $TEMP_DIR${R}" >&2
    exit 1
fi
...
_dl() {
    # _dl <url> <dest>  – shows progress bar
    local url="$1" dest="$2"
    local tmpfile

    if command -v mktemp &>/dev/null; then
        tmpfile="$(mktemp -p "$TEMP_DIR" -t v2auto.XXXXXX)"
    else
        tmpfile="${TEMP_DIR}/v2auto.$$.tmp"
    fi
    ...
    if command -v curl &>/dev/null; then
        curl -L -f --retry 3 --retry-delay 2 -# -o "$tmpfile" "$url" 2>/dev/tty
        _rc=$?
    elif command -v wget &>/dev/null; then
        wget --tries=3 -O "$tmpfile" "$url" 2>/dev/tty
        _rc=$?
    else
    ...
    if [[ $_rc -eq 0 ]] && [[ -s "$tmpfile" ]]; then
        mv "$tmpfile" "$dest"
        return 0
    fi

    rm -f "$tmpfile"
    _err "Download failed (HTTP error or empty file): $url"
    return 1
}

_dl_q() {
    # _dl_q <url> <dest> – quiet download (no progress)
    local url="$1" dest="$2"
    local tmpfile
    ...
    if command -v curl &>/dev/null; then
        curl -L -f -s --retry 2 --retry-delay 1 -o "$tmpfile" "$url"
        _rc=$?
    elif command -v wget &>/dev/null; then
        wget -q --tries=2 -O "$tmpfile" "$url"
        _rc=$?
    else
        _err "Neither curl nor wget found."
        return 1
...
if $_INSTALL_XRAY; then
    ...
    XRAY_TMP="${TEMP_DIR}/${XRAY_ZIP}"
    _info "Downloading ${XRAY_ZIP}..."
    _dl "$XRAY_URL" "$XRAY_TMP"

    _info "Extracting..."
    _TMP_X=$(mktemp -d -p "$TEMP_DIR" -t v2auto.XXXXXX)

...
if $IS_TERMUX; then
    ...
    if _ask "Auto-start V2auto when Termux opens?" "n"; then
        {
            printf '\n# >>> v2auto-autostart <<<\n'
            printf 'if [[ $- == *i* ]] && [[ -z "${V2AUTO_STARTED:-}" ]]; then\n'
            printf '    export V2AUTO_STARTED=1\n'
            printf '    (cd "%s" && python3 v2web.py > "${TMPDIR:-/tmp}/v2auto.log" 2>&1 &)\n' "$INSTALL_DIR"
            printf '    echo -e "\\033[0;36m[*] V2auto started -> http://127.0.0.1:8080\\033[0m"\n'
            printf 'fi\n'
            printf '# <<< v2auto-autostart <<<\n'
        } >> "$HOME/.bashrc"
        ...
        if [[ "$MODE" == "update" ]] && [[ -f "$SVC_FILE" ]]; then
            ...
            if _ask "Install V2auto as a systemd service?" "n"; then
                ...
                if systemctl is-active --quiet "$SVC"; then
                    ...
                    _warn "Check: systemctl status $SVC"
                fi
                echo ""
                echo -e "  ${DM}systemctl {status|stop|restart} $SVC${R}"
                echo -e "  ${DM}journalctl -u $SVC -f${R}"
            else
...
_check() {
    ...
    if [[ -e "$path" ]]; then
        ...
        if [[ "$required" == "true" ]]; then
            _err "$label — NOT FOUND"; FAIL=$((FAIL + 1))
        else
...
echo ""
if [[ $FAIL -eq 0 ]]; then
    echo -e "  ${GN}${BD}$( [[ "$MODE" == "update" ]] && echo "Update" || echo "Installation" ) complete!  (${PASS} files OK)${R}"
else
...
echo ""
echo -e "    Browser: ${BD}http://localhost:8080${R}"
echo ""
echo -e "  ${DM}Apply alias in this session:  source ${RC_FILES[0]:-$HOME/.bashrc}${R}"
echo ""
_line
echo ""

...
<img src="./v2auto_banner.jpeg" alt="V2auto banner" width="600"/>
...
```
v2auto/
├── v2auto.py          ← pipeline engine
├── v2web.py           ← web dashboard
├── xray               ← xray-core binary
├── geoip.dat          ← IP routing rules
├── geosite.dat        ← domain routing rules
├── vless_working.txt  ← saved working configs
└── v2auto_uuid.txt    ← persistent VLESS inbound UUID
```
...
To update project files and dependencies, just run the installer again:

```bash
curl -fsSL https://raw.githubusercontent.com/akarezi/V2auto/main/install.sh | bash
```

v2auto.py and v2web.py are always re-downloaded. xray and geo files are only updated if you confirm.
